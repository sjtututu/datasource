...
class Alpha101(object):
    ...
    def __init__(self, startdate, enddate, count, length):
        ...
        self.vwap = stock_price['amount'] / \
            (stock_price['volume'] + 0.001)  # 分母防止被0整除
        self.length = length
    ...
    def alpha001(self):
        ...
        save_hdf(alpha001, 'alpha001', self.length)
        return

    # Alpha#2	 (-1 * correlation(rank(delta(log(volume), 2)), rank(((close - open) / open)), 6))
    def alpha002(self):
        alpha002 = -1 * correlation(rank(delta(log(self.volume), 2)),
                                    rank((self.close - self.open) / self.open), 6)
        save_hdf(alpha002, 'alpha002', self.length)
        return

    # Alpha#3	 (-1 * correlation(rank(open), rank(volume), 10))
    def alpha003(self):
        alpha003 = -1 * correlation(rank(self.open), rank(self.volume), 10)
        save_hdf(alpha003, 'alpha003', self.length)
        return

    # Alpha#4	 (-1 * Ts_Rank(rank(low), 9))
    def alpha004(self):
        alpha004 = -1 * ts_rank(rank(self.low), 9)
        save_hdf(alpha004, 'alpha004', self.length)
        return
    ...
    def alpha005(self):
        ...
        save_hdf(alpha005, 'alpha005', self.length)
        return

    # Alpha#6	 (-1 * correlation(open, volume, 10))
    def alpha006(self):
        alpha006 = -1 * correlation(self.open, self.volume, 10)
        save_hdf(alpha006, 'alpha006', self.length)
        return
    ...
    def alpha007(self):
        ...
        save_hdf(alpha007, 'alpha007', self.length)
        return
    ...
    def alpha008(self):
        ...
        save_hdf(alpha008, 'alpha008', self.length)
        return
    ...
    def alpha009(self):
        ...
        save_hdf(alpha009, 'alpha009', self.length)
        return
    ...
    def alpha010(self):
        ...
        save_hdf(alpha010, 'alpha010', self.length)
        return
    ...
    def alpha011(self):
        ...
        save_hdf(alpha011, 'alpha011', self.length)
        return

    # Alpha#12	 (sign(delta(volume, 1)) * (-1 * delta(close, 1)))
    def alpha012(self):
        alpha012 = sign(delta(self.volume, 1)) * (-1 * delta(self.close, 1))
        save_hdf(alpha012, 'alpha012', self.length)
        return
    ...
    def alpha013(self):
        alpha013 = -1 * rank(covariance(rank(self.close), rank(self.volume),
                                        5))
        save_hdf(alpha013, 'alpha013', self.length)
        return
    ...
    def alpha014(self):
        ...
        save_hdf(alpha014, 'alpha014', self.length)
        return
    ...
    def alpha015(self):
        ...
        save_hdf(alpha015, 'alpha015', self.length)
        return

    # Alpha#16	 (-1 * rank(covariance(rank(high), rank(volume), 5)))
    def alpha016(self):
        alpha016 = -1 * rank(covariance(rank(self.high), rank(self.volume), 5))
        save_hdf(alpha016, 'alpha016', self.length)
        return
    ...
    def alpha017(self):
        ...
        save_hdf(alpha017, 'alpha017', self.length)
        return
    ...
    def alpha018(self):
        ...
        save_hdf(alpha018, 'alpha018', self.length)
        return
    ...
    def alpha019(self):
        ...
        save_hdf(alpha019, 'alpha019', self.length)
        return
    ...
    def alpha020(self):
        ...
        save_hdf(alpha020, 'alpha020', self.length)
        return

    # Alpha#21	 ((((sum(close, 8) / 8) + stddev(close, 8)) < (sum(close, 2) / 2)) ? (-1 * 1) : (((sum(close,2) / 2) < ((sum(close, 8) / 8) - stddev(close, 8))) ? 1 : (((1 < (volume / adv20)) || ((volume /adv20) == 1)) ? 1 : (-1 * 1))))
    def alpha021(self):
        cond_1 = mean(self.close, 8) + \
            stddev(self.close, 8) < mean(self.close, 2)
        ...
        save_hdf(alpha021, 'alpha021', self.length)
        return
    ...
    def alpha022(self):
        ...
        save_hdf(alpha022, 'alpha022', self.length)
        return
    ...
    def alpha023(self):
        ...
        save_hdf(alpha023, 'alpha023', self.length)
        return

    # Alpha#24	 ((((delta((sum(close, 100) / 100), 100) / delay(close, 100)) < 0.05) ||((delta((sum(close, 100) / 100), 100) / delay(close, 100)) == 0.05)) ? (-1 * (close - ts_min(close,100))) : (-1 * delta(close, 3)))
    def alpha024(self):
        cond = delta(mean(self.close, 100), 100) / delay(self.close,
                                                         100) <= 0.05
        alpha024 = -1 * delta(self.close, 3)
        alpha024[cond] = -1 * (self.close - ts_min(self.close, 100))
        save_hdf(alpha024, 'alpha024', self.length)
        return
    ...
    def alpha025(self):
        ...
        save_hdf(alpha025, 'alpha025', self.length)
        return
    ...
    def alpha026(self):
        ...
        save_hdf(alpha026, 'alpha026', self.length)
        return
    ...
    def alpha027(self):
        ...
        alpha027[alpha027 <= 0.5] = 1
        save_hdf(alpha027, 'alpha027', self.length)
        return
    ...
    def alpha028(self):
        ...
        save_hdf(alpha028, 'alpha028', self.length)
        return
    ...
    def alpha029(self):
        ...
        save_hdf(alpha029, 'alpha029', self.length)
        return
    ...
    def alpha030(self):
        ...
        alpha030 = ((1.0 - rank(inner)) * ts_sum(self.volume, 5)) / ts_sum(
            self.volume, 20)
        save_hdf(alpha030, 'alpha030', self.length)
        return
    ...
    def alpha031(self):
        ...
        p1 = rank(
            rank(
                rank(decay_linear((-1 * rank(rank(delta(self.close, 10)))),
                                  10))))
        p2 = rank((-1 * delta(self.close, 3)))
        p3 = sign(scale(df))
        alpha031 = p1 + p2 + p3
        save_hdf(alpha031, 'alpha031', self.length)
        return
    ...
    def alpha032(self):
        ...
        save_hdf(alpha032, 'alpha032', self.length)
        return

    # Alpha#33	 rank((-1 * ((1 - (open / close))^1)))
    def alpha033(self):
        alpha033 = rank(-1 + (self.open / self.close))
        save_hdf(alpha033, 'alpha033', self.length)
        return
    ...
    def alpha034(self):
        ...
        save_hdf(alpha034, 'alpha034', self.length)
        return
    ...
    def alpha035(self):
        ...
        save_hdf(alpha035, 'alpha035', self.length)
        return
    ...
    def alpha036(self):
        ...
        save_hdf(alpha036, 'alpha036', self.length)
        return
    ...
    def alpha037(self):
        ...
        save_hdf(alpha037, 'alpha037', self.length)
        return
    ...
    def alpha038(self):
        ...
        save_hdf(alpha038, 'alpha038', self.length)
        return
    ...
    def alpha039(self):
        ...
        save_hdf(alpha039, 'alpha039', self.length)
        return
    ...
    def alpha040(self):
        ...
        save_hdf(alpha040, 'alpha040', self.length)
        return

    # Alpha#41	 (((high * low)^0.5) - vwap)
    def alpha041(self):
        alpha041 = pow((self.high * self.low), 0.5) - self.vwap
        save_hdf(alpha041, 'alpha041', self.length)
        return
    ...
    def alpha042(self):
        ...
        save_hdf(alpha042, 'alpha042', self.length)
        return
    ...
    def alpha043(self):
        ...
        save_hdf(alpha043, 'alpha043', self.length)
        return
    ...
    def alpha044(self):
        ...
        save_hdf(alpha044, 'alpha044', self.length)
        return
    ...
    def alpha045(self):
        ...
        save_hdf(alpha045, 'alpha045', self.length)
        return
    ...
    def alpha046(self):
        ...
        save_hdf(alpha046, 'alpha046', self.length)
        return
    ...
    def alpha047(self):
        ...
        save_hdf(alpha047, 'alpha047', self.length)
        return
    ...
    def alpha049(self):
        ...
        save_hdf(alpha049, 'alpha049', self.length)
        return
    ...
    def alpha050(self):
        ...
        save_hdf(alpha050, 'alpha050', self.length)
        return
    ...
    def alpha051(self):
        ...
        save_hdf(alpha051, 'alpha051', self.length)
        return
    ...
    def alpha052(self):
        ...
        save_hdf(alpha052, 'alpha052', self.length)
        return
    ...
    def alpha053(self):
        ...
        save_hdf(alpha053, 'alpha053', self.length)
        return
    ...
    def alpha054(self):
        ...
        alpha054 = -1 * (self.low - self.close) * (self.open **
                                                   5) / (inner *
                                                         (self.close**5))
        save_hdf(alpha054, 'alpha054', self.length)
        return
    ...
    def alpha055(self):
        ...
        save_hdf(alpha055, 'alpha055', self.length)
        return
    ...
    def alpha057(self):
        ...
        save_hdf(alpha057, 'alpha057', self.length)
        return
    ...
    def alpha060(self):
        ...
        save_hdf(alpha060, 'alpha060', self.length)
        return
    ...
    def alpha061(self):
        ...
        save_hdf(alpha061, 'alpha061', self.length)
        return
    ...
    def alpha062(self):
        ...
        save_hdf(alpha062, 'alpha062', self.length)
        return
    ...
    def alpha064(self):
        ...
        save_hdf(alpha064, 'alpha064', self.length)
        return
    ...
    def alpha065(self):
        ...
        save_hdf(alpha065, 'alpha065', self.length)
        return
    ...
    def alpha066(self):
        ...
        save_hdf(alpha066, 'alpha066', self.length)
        return
    ...
    def alpha068(self):
        ...
        save_hdf(alpha068, 'alpha068', self.length)
        return
    ...
    def alpha071(self):
        ...
        alpha071 = pd.DataFrame(np.maximum(p1, p2),
                                index=self.close.index,
                                columns=self.close.columns)
        save_hdf(alpha071, 'alpha071', self.length)
        return
    ...
    def alpha072(self):
        ...
        save_hdf(alpha072, 'alpha072', self.length)
        return
    ...
    def alpha073(self):
        ...
        alpha073 = (-1 * pd.DataFrame(np.maximum(p1, p2),
                                      index=self.close.index,
                                      columns=self.close.columns))
        save_hdf(alpha073, 'alpha073', self.length)
        return
    ...
    def alpha074(self):
        ...
        save_hdf(alpha074, 'alpha074', self.length)
        return
    ...
    def alpha075(self):
        ...
        save_hdf(alpha075, 'alpha075', self.length)
        return
    ...
    def alpha077(self):
        ...
        alpha077 = (pd.DataFrame(np.minimum(p1, p2),
                                 index=self.close.index,
                                 columns=self.close.columns))
        save_hdf(alpha077, 'alpha077', self.length)
        return
    ...
    def alpha078(self):
        ...
        save_hdf(alpha078, 'alpha078', self.length)
        return
    ...
    def alpha081(self):
        ...
        alpha081 = ((rank(log(product(rank((rank(correlation(self.vwap, ts_sum(adv10, 50), 8)).pow(
            4))), 15))) < rank(correlation(rank(self.vwap), rank(self.volume), 5))) * -1)
        save_hdf(alpha081, 'alpha081', self.length)
        return
    ...
    def alpha083(self):
        alpha083 = ((rank(
            ...
            (((self.high - self.low) / (ts_sum(self.close, 5) / 5)) /
             (self.vwap - self.close)))
        save_hdf(alpha083, 'alpha083', self.length)
        return
    ...
    def alpha084(self):
        ...
        save_hdf(alpha084, 'alpha084', self.length)
        return
    ...
    def alpha085(self):
        ...
        save_hdf(alpha085, 'alpha085', self.length)
        return

    # Alpha#86	 ((Ts_Rank(correlation(close, sum(adv20, 14.7444), 6.00049), 20.4195) < rank(((open+ close) - (vwap + open)))) * -1)
    def alpha086(self):
        adv20 = mean(self.volume, 20)
        alpha086 = ((ts_rank(correlation(self.close, mean(adv20, 15), 6), 20) < rank(
            ((self.open + self.close) - (self.vwap + self.open)))) * -1)
        save_hdf(alpha086, 'alpha086', self.length)
        return
    ...
    def alpha088(self):
        ...
        p2 = ts_rank(decay_linear(correlation(
            ts_rank(self.close, 8), ts_rank(adv60, 21), 8), 7), 3)
        alpha088 = (pd.DataFrame(np.minimum(p1, p2),
                                 index=self.close.index, columns=self.close.columns))
        save_hdf(alpha088, 'alpha088', self.length)
        return
    ...
    def alpha092(self):
        ...
        p2 = ts_rank(decay_linear(correlation(
            rank(self.low), rank(adv30), 8), 7), 7)
        alpha092 = (pd.DataFrame(np.minimum(p1, p2),
                                 index=self.close.index, columns=self.close.columns))
        save_hdf(alpha092, 'alpha092', self.length)
        return
    ...
    def alpha094(self):
        adv60 = mean(self.volume, 60)
        alpha094 = ((rank((self.vwap - ts_min(self.vwap, 12))).pow(
            ts_rank(correlation(ts_rank(self.vwap, 20), ts_rank(adv60, 4), 18), 3)) * -1))
        save_hdf(alpha094, 'alpha094', self.length)
        return

    # Alpha#95	 (rank((open - ts_min(open, 12.4105))) < Ts_Rank((rank(correlation(sum(((high + low)/ 2), 19.1351), sum(adv40, 19.1351), 12.8742))^5), 11.7584))
    def alpha095(self):
        adv40 = mean(self.volume, 40)
        alpha095 = ((rank((self.open - ts_min(self.open, 12))) < ts_rank((rank(correlation(
            mean(((self.high + self.low) / 2), 19), mean(adv40, 19), 13)).pow(5)), 12))*1)
        save_hdf(alpha095, 'alpha095', self.length)
        return

    # Alpha#96	 (max(Ts_Rank(decay_linear(correlation(rank(vwap), rank(volume), 3.83878),4.16783), 8.38151), Ts_Rank(decay_linear(Ts_ArgMax(correlation(Ts_Rank(close, 7.45404),Ts_Rank(adv60, 4.13242), 3.65459), 12.6556), 14.0365), 13.4143)) * -1)
    def alpha096(self):
        adv60 = mean(self.volume, 60)
        p1 = ts_rank(decay_linear(correlation(
            rank(self.vwap), rank(self.volume), 4), 4), 8)
        p2 = ts_rank(decay_linear(ts_argmax(correlation(
            ts_rank(self.close, 7), ts_rank(adv60, 4), 4), 13), 14), 13)
        alpha096 = (-1 * pd.DataFrame(np.maximum(p1, p2),
                                      index=self.close.index, columns=self.close.columns))
        save_hdf(alpha096, 'alpha096', self.length)
        return
    ...
    def alpha098(self):
        ...
        adv15 = mean(self.volume, 15)
        alpha098 = (rank(decay_linear(correlation(self.vwap, mean(adv5, 26), 5), 7)) - rank(
            decay_linear(ts_rank(ts_argmin(correlation(rank(self.open), rank(adv15), 21), 9), 7), 8)))
        save_hdf(alpha098, 'alpha098', self.length)
        return

    # Alpha#99	 ((rank(correlation(sum(((high + low) / 2), 19.8975), sum(adv60, 19.8975), 8.8136)) <rank(correlation(low, volume, 6.28259))) * -1)
    def alpha099(self):
        adv60 = mean(self.volume, 60)
        alpha099 = ((rank(correlation(ts_sum(((self.high + self.low) / 2), 20),
                                      ts_sum(adv60, 20), 9)) < rank(correlation(self.low, self.volume, 6))) * -1)
        save_hdf(alpha099, 'alpha099', self.length)
        return
    ...
    def alpha101(self):
        ...
        save_hdf(alpha101, 'alpha101', self.length)
        return
...
def run_func(paras):
    ...
    count = paras["count"]
    length = paras["length"]
    alphas = Alpha101(startdate, enddate, count, length)
    func_list = paras["func"]
    for func_name in func_list:
        eval("alphas." + func_name + "()")
    return


def set_params(func_list, Start_Date, End_Date, count, length):
    td = {"module_name": "Alpha101",
...
if __name__ == '__main__':
    ...
    Start_Date = None
    End_Date = "20191231"  # str(TradeDate.iloc[-1, 0])
    count = 600  # 用于计算因子的数据长度
    length = TradeDate.shape[0]  # 输出更新的数据长度
    alphas = Alpha101(Start_Date, End_Date, count, length)
    func_list = [x for x in dir(alphas) if not x.startswith("__")]
    func_list = func_list[0:82]  # 截取有效函数
